关于[Autoform](https://github.com/aldeed/meteor-autoform)
====
...
## 主要组成  
主要是通过 [simple-schema](https://github.com/aldeed/meteor-simple-schema) 来映射实体。
...
AutoForm 是一个包含UI组件和helpers来自动创建基本的包含插入和更新事件的表单、且自动进行验证的一个meteor包。这个包依赖于simple-schema并且会自动安装这包。你也可以选择添加collection2到你自己的app中。
## NOTE: AutoForm 5.0
...
AutoForm 5.0现在已经可以使用，做了很多兼容的工作，但是也有很多修复和改进。这个版本内部包含大量的重构。所以升级到5.0以上是一个很好的选择。更多明细可以查看[change log](https://github.com/aldeed/meteor-autoform/blob/master/CHANGELOG.md)
...
**Add-on Package Authors**: 请使用 将`api.use` 改写成 `api.use('aldeed:autoform@4.0.0 || 5.0.0')` 来升级。 

## 目录
<!-- START doctoc generated TOC please keep comment here to allow auto update -->
<!-- DON'T EDIT THIS SECTION, INSTEAD RE-RUN doctoc TO UPDATE -->
**目录**  *通过 [DocToc](https://github.com/thlorenz/doctoc)自动生成*
...
通过collection2提供的架构支持创建一个Mongo.Collection的实例。(添加autoform的时候不会自动添加collection2这个包，需要通过meteor add aldeed:collection2来添加)
...
*如果你移除了insecure包，就必须要做安全验证，可以使用[ongoworks:security](https://atmospherejs.com/ongoworks/security)。*
### A Basic Insert Form
...
* 使用bootstrap3来自动生成表单。
* 将Books架构中的所有键生成为html5字段。
* 一个提交按钮，将收集到的字段信息插入到Books集合中。
* 
